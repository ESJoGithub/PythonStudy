...
class Widget():
  ...
  def get_widget(self, title=None, count=1, mode="widget"):
    ...
    label1 = tk.Label(frame1, text=self.name, height=1, padx=10, pady=2, background="white")
    label1.pack(side="left")    
    label1.bind("<Button-1>", f_Event.click)
    label1.bind("<B1-Motion>", f_Event.move)
    label1.bind("<ButtonRelease-1>", f_Event.release)  
    button = tk.Button(frame1, text="X", height=1, relief="flat", overrelief="flat", command=f_Event.close, background="white")
